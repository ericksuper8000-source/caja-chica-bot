...
SYSTEM_PROMPT_PARSE = (
    ...
    "- Un monto de 0 (cero) NO es un monto válido: si la frase no menciona dinero, el "
    ...
    "'corrección', 'cambia', 'era X no Y', 'no eran 5 eran 6'). El resultado es un DELTA: "
    "llena SOLO los campos que el usuario corrigió o mencionó explícitamente (ej: 'eran 6 "
    "rojos no 5' → monto=6000). Todo campo que el usuario NO mencione va en null, porque el "
    "sistema conserva el valor anterior de esa transacción. NUNCA inventes un valor para "
    "evitar el null: está prohibido adivinar 'Otros' u otra categoría sin que la frase la "
    "mencione. Solo infiere del contexto cuando la frase lo permita (ej: 'el almuerzo' → "
    "categoria=Alimentación, detalle=Almuerzo). Si el usuario restablece la transacción "
    "completa, llena todos los campos.\n"
