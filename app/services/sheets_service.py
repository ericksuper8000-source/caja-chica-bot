...
def get_sheets_client() -> gspread.Client:
    ...
    """
    try:
        # gspread.service_account retorna una instancia de gspread.Client
...
def _sync_append_row(spreadsheet_id: str, row_values: List[Any]) -> None:
    ...
    """
    client: gspread.Client = get_sheets_client()
    ...
    worksheet: gspread.Worksheet = spreadsheet.get_worksheet(0)
    # Inserta la fila
...
async def append_transaction_to_sheet(transaction_data: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        monto: float = float(transaction_data.get("monto", 0))
        tipo: str = str(transaction_data.get("tipo_movimiento", "Gasto"))
        ...
        # 3. Delegación asíncrona a un hilo secundario
